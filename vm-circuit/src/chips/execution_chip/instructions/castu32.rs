...
impl<F: FieldExt> InstructionGadget<F> for CastU32<F> {
    ...
    fn configure(&self, cells: &StepChipCells<F>, cb: &mut ConstraintBuilder<F>) {
        ...
        // x = out
        cb.add_constraint("cast u32", x - out.clone());
        // range check for out
        let bytes_8 = FieldBytes::from(self.bytes.clone()).expr_with_n(NUM_OF_BYTES_U32);
        cb.add_constraint("cast u32 range check", out - bytes_8);
