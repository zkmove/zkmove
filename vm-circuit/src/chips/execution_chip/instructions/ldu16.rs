...
#[derive(Clone, Debug)]
pub struct LdU16<F: FieldExt> {
    value_a: Cell<F>,
}

impl<F: FieldExt> InstructionGadget<F> for LdU16<F> {
    const NAME: &'static str = "LDU16";

    const OPCODE: Opcode = Opcode::LdU16;
    fn configure(&self, cells: &StepChipCells<F>, cb: &mut ConstraintBuilder<F>) {
        //LdU16

        LoadOp::constrain_ld_op(cells, cb);
        LoadOp::lookup_ld_op(cb, cells, &self.value_a);
        LookupBytecode::lookup_bytecode(cb, cells, Opcode::LdU16, self.value_a.expression.clone());
    }
