...
impl<F: Field> InstructionGadget<F> for VecPopBack<F> {
    ...
    fn configure(&self, cells: &StepChipCells<F>, cb: &mut ConstraintBuilder<F>) {
        ...
        let curr_header_idx = headers_count - 1u64.expr();
        for i in 0..MAX_ADDRESS_EXT_LENGTH {
            ...
            //
            //     let len_diff = 1u64.expr() * 2u64.pow(16).expr(); //vector length decrease one
    ...
    fn assign(
        ...
        for i in 0..headers_count {
            ...
            self.new_headers_value[i].assign(region, offset, op.value().field_value())?;
        }
