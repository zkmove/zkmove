...
impl<F: FieldExt> InstructionGadget<F> for Eq<F> {
    ...
    fn configure(&self, cells: &StepChipCells<F>, cb: &mut ConstraintBuilder<F>) {
        ...
        let gc_expr = cells.gc.expression.clone() - cb.next.cells.gc.expression.clone()
            + flattened_value_len_a.clone() + flattened_value_len_b.clone()
            + 2.expr(); // bool result
        let module_index =
        ...
        bcb.require_boolean("result is bool", self.result.cells.value().expression.clone());
            ...
            cb.condition(
                ...
                |cb| {
                    cb.add_lookup(
                        "eq(pop left)",
                        RWLookup::stack_pop(
                            cells.gc.expression.clone() + flattened_value_len_a.clone() + (i as u64).expr(),
        ...
        for (i, item) in self.value_a.cells.word.iter().enumerate() {
            cb.condition(
                (1.expr() - self.value_a.cells.word_mask[i].expression.clone())
                    * (1.expr() - is_reference.clone()) * self.result.cells.value().expression.clone(),
                |cb| {
        ...
        cb.condition(
            ...
            |cb| {
                ...
                cb.add_lookup(
                    "eq(left unequal row)",
                    RWLookup {
                        gc: cells.gc.expression.clone() + flattened_value_len_a.clone() + unequal_row.clone(),
                        rw_target: (RWTarget::Stack as u64).expr(),
                        rw: (RW::READ as u64).expr(),
                        frame_index: 0.expr(),
                ...
                cb.condition(1.expr() - unequal_column.clone(), |cb| {
                    ...
                    let constraint_1 = ((self.unequal_row_addr_ext_a.expression.clone() - self.unequal_row_addr_ext_b.expression.clone()) * self.delta_invert.expression.clone() - 1.expr())
                        * (self.unequal_row_addr_ext_a.expression.clone() - self.unequal_row_addr_ext_b.expression.clone());
                    // constrain "unequal_row_addr_ext_a != unequal_row_addr_ext_b"
                    let constraint_2 = (self.unequal_row_addr_ext_a.expression.clone() - self.unequal_row_addr_ext_b.expression.clone()) * self.delta_invert.expression.clone() - 1.expr();
                ...
                cb.condition(unequal_column.clone(), |cb| {
                    // constrain delta_invert
                    let constraint_1 = ((self.unequal_row_value_a.expression.clone() - self.unequal_row_value_b.expression.clone()) * self.delta_invert.expression.clone() - 1.expr())
                        * (self.unequal_row_value_a.expression.clone() - self.unequal_row_value_b.expression.clone());
                    let constraint_2 = (self.unequal_row_value_a.expression.clone() - self.unequal_row_value_b.expression.clone()) * self.delta_invert.expression.clone() - 1.expr();
        ...
        let write = RWLookup::stack_push(
            cells.gc.expression.clone() + flattened_value_len_a.clone() + flattened_value_len_b.clone(),
            cells.stack_size.expression.clone() - 2.expr(),
            0.expr(),
            ValueHeader::default_for_simple().expr(),
        );
        cb.add_lookup("eq(push result header)", write);
        let write = RWLookup::stack_push(
            cells.gc.expression.clone() + flattened_value_len_a.clone() + flattened_value_len_b.clone() + 1.expr(),
            cells.stack_size.expression.clone() - 2.expr(),
            1.expr(),
            self.result.cells.value().expression.clone(),
        );
        cb.add_lookup("eq(push result value)", write);
    ...
    fn assign(
        ...
        self.value_a
            .assign(region, offset, rw_operations, step.gc, flattened_value_len_a)?;
        self.value_b
            .assign(region, offset, rw_operations, step.gc + flattened_value_len_a, flattened_value_len_b)?;
        self.result
            .assign(region, offset, rw_operations, step.gc + flattened_value_len_a + flattened_value_len_b)?;

        // assign unequal_row_xxx, delta_invert
        let result_op = rw_operations.0.get(step.gc + flattened_value_len_a + flattened_value_len_b + 1).ok_or(Error::Synthesis)?;

        if result_op.value().value() == Some(F::zero()) { // a and b are not equal
            let unequal_row =
            ...
            let unequal_op_a = rw_operations.0.get(step.gc + unequal_row).ok_or(Error::Synthesis)?;
            let unequal_op_b = rw_operations.0.get(step.gc + flattened_value_len_a + unequal_row).ok_or(Error::Synthesis)?;
            ...
            self.unequal_row_addr_ext_a.assign(region, offset, Some(addr_ext_a))?;
            self.unequal_row_addr_ext_b.assign(region, offset, Some(addr_ext_b))?;
            self.unequal_row_value_a.assign(region, offset, Some(val_a))?;
            self.unequal_row_value_b.assign(region, offset, Some(val_b))?;
            match unequal_column {
                0 => {
                    self.delta_invert.assign(region, offset, addr_ext_a.delta_invert(addr_ext_b))?;
                },
                1 => {
                    self.delta_invert.assign(region, offset, val_a.delta_invert(val_b))?;
                },
                _ => {},
            };
        }
