...
use logger::prelude::*;
use movelang::value::NUM_OF_BYTES_U16;
...
impl<F: FieldExt> InstructionGadget<F> for CastU16<F> {
    const NAME: &'static str = "CASTU16";
    ...
    fn configure(&self, cells: &StepChipCells<F>, cb: &mut ConstraintBuilder<F>) {
        ...
        cb.add_constraint("cast u16 hi", out_hi);
        cb.add_constraint("cast u16 lo", input_lo - out_lo.clone());
        // range check for out. u16 at out_lo
        let bytes_2 = FieldBytes::from(self.bytes.clone()).expr_with_n(NUM_OF_BYTES_U16);
        ...
        LookupBytecode::lookup_bytecode(cb, cells, Opcode::CastU16, 0.expr());
    }
