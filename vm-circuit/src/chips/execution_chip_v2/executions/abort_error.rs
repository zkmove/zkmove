...
#[derive(Clone, Debug)]
pub struct AbortOrError<F> {
    phantom_data: PhantomData<F>,
}

impl<F: Field> InstructionGadgetV2<F> for AbortOrError<F> {
    const NAME: &'static str = "AbortOrError";
    const EXECUTION_STATE: ExecutionState = ExecutionState::AbortOrError;

    fn configure(cb: &mut ConstraintBuilderV2<F>) -> Self {
        let step_curr = cb.curr.state.clone();
        cb.require_zero(
            "opcode = 0 | Opcodes::ABORT",
            step_curr.opcode.expr() * ((Opcodes::ABORT as u64).expr() - step_curr.opcode.expr()),
        );
