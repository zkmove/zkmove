...
impl<F: Field> InstructionGadgetV2<F> for VecPopBackStage1<F> {
    ...
    fn configure(cb: &mut ConstraintBuilderV2<F>) -> Self {
        cb.last_row(|cb| {
            cb.require_next_state(ExecutionState::VecPopBackStage2);
        });
...
impl<F: Field> VecPopBackStage2<F> {
    ...
    const NEXT_STATE: ExecutionState = ExecutionState::VecPopBackStage3;
}
...
#[derive(Clone)]
pub struct VecPopBackStage3<F> {
    vector_sub_index: Cell<F>,
    extended_vector_sub_index: ExtendedSubIndex<F, 8>,
    vector_origin_len: Cell<F>,
    elem_len: Cell<F>,
}
impl<F: Field> VecPopBackStage3<F> {
    const PREV_STATE: ExecutionState = ExecutionState::VecPopBackStage2;
}
impl<F: Field> InstructionGadgetV2<F> for VecPopBackStage3<F> {
    const NAME: &'static str = "VecPopBackStage3";
    const OPCODE: Opcode = Opcode::VecPopBack;
    const EXECUTION_STATE: ExecutionState = ExecutionState::VecPopBackStage3;
    ...
    fn configure(cb: &mut ConstraintBuilderV2<F>) -> Self {
        ...
        cb.require_equal(
            ...
            step_curr.local_sub_index.expr(),
            extended_vector_sub_index.concat_sub_index(vector_origin_len.expr() + step_curr.stack_push_sub_index.expr()*DEPTH_POW_OF_ONE_LEVEL.expr())
        );
        cb.first_row(|cb| {
