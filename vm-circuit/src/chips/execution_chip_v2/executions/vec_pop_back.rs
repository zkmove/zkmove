...
impl<F: Field> InstructionGadgetV2<F> for VecPopBackStage1<F> {
    ...
    const OPCODES: &'static [Opcode] = &[Opcode::VecPopBack];
        ...
        cb.not_first_row(|cb| {
            ...
            cb.last_row(|cb| {
                cb.require_equal(
                    ...
                    step_prev.local_read_value.as_header().flen(),
                    step_prev.local_write_value.as_header().flen() + step_next.step_counter.expr()
                );
            });
        });
        cb.last_row(|cb| {
