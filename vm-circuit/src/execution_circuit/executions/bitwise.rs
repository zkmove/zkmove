    ...
    fn configure(cb: &mut VmConstraintBuilder<F>) -> Self {
        ...
        cb.last_row(|cb| {
            ...
            let nibbles_lhs = (1..(R as i32))
                ...
                .collect::<Vec<_>>();
            cb.require_equal(
                format!("{}, lhs.lo = from_limbs(nibbles_lhs[0..32])", Self::NAME),
                lhs.lo(),
                from_limbs::expr::<_, _, 4>(&nibbles_lhs[..NUM_OF_BYTES_U256]),
            );
            cb.require_equal(
                format!("{}, lhs.hi = from_limbs(nibbles_lhs[32..])", Self::NAME),
                lhs.hi(),
                from_limbs::expr::<_, _, 4>(&nibbles_lhs[NUM_OF_BYTES_U256..]),
            );
            ...
            let nibbles_rhs = (1..(R as i32))
                ...
                .collect::<Vec<_>>();
            cb.require_equal(
                format!("{}, rhs.lo = from_limbs(nibbles_rhs[0..32])", Self::NAME),
                rhs.lo(),
                from_limbs::expr::<_, _, 4>(&nibbles_rhs[..NUM_OF_BYTES_U256]),
            );
            cb.require_equal(
                format!("{}, rhs.hi = from_limbs(nibbles_rhs[32..])", Self::NAME),
                rhs.hi(),
                from_limbs::expr::<_, _, 4>(&nibbles_rhs[NUM_OF_BYTES_U256..]),
            );
            ...
            cb.require_equal(
                format!("{}, out.lo = from_limbs(nibbles[0..32])", Self::NAME),
                out.lo(),
                from_limbs::expr::<_, _, 4>(&nibbles_out[..NUM_OF_BYTES_U256]),
            );
            cb.require_equal(
                format!("{}, out.hi = from_limbs(nibbles[32..])", Self::NAME),
                out.hi(),
                from_limbs::expr::<_, _, 4>(&nibbles_out[NUM_OF_BYTES_U256..]),
            );
        });
...
impl<F: Field, const R: usize, const C: usize> InstructionGadgetV2<F> for BitwiseStage2<F, R, C> {
    ...
    fn configure(cb: &mut VmConstraintBuilder<F>) -> Self {
        ...
        cb.first_row(|cb| {
            ...
            cb.require_equal(
                format!("{}, step_counter(0) == R", Self::NAME),
                step_curr.step_counter.expr(),
                R.expr(),
            );
        });
...
impl<F: Field> LookupBitwise<F> {
    fn lookup<const C: usize>(cb: &mut VmConstraintBuilder<F>, op: BitwiseOperation<F, C>) {
