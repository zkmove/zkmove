...
impl<F: Field> InstructionGadgetV2<F> for AddSub<F> {
    ...
    fn configure(cb: &mut VmConstraintBuilder<F>) -> Self {
        ...
        cb.first_row(|cb| {
            cb.require_in_set(
                format!("{}, operand0(0) in set INTEGER_NUM_OF_BYTES", Self::NAME),
        ...
        cb.last_row(|cb| {
            ...
            let add_sub = AddSubGadget::construct(
                cb,
                is_add.expr(),
                step_curr.operand0.expr(),
                lhs,
...
impl<F: Field> AddSubGadget<F> {
    fn construct(
        ...
        cb.condition(is_u8.expr(), |cb| {
            ...
            cb.require_equal(
                "overflow == !in_range(out_lo)".to_string(),
                overflow.expr(),
                1u64.expr() - in_range,
            );
        });
        ...
        cb.condition(is_u128.expr(), |cb| {
            ...
            cb.require_true("out_lo < 2^128".to_string(), in_range);
            //OVERFLOW if out_hi == 1
            cb.require_in_set(
                "out_hi == 0 | 1".to_string(),
                out.hi(),
                (0u64..2).map(|v| v.expr()).collect(),
            );
            cb.require_equal("overflow == out_hi".to_string(), overflow.expr(), out.hi());
        });
        ...
        cb.condition(is_u256.expr(), |cb| {
            ...
            cb.require_true("out_lo < 2^128".to_string(), in_range_lo);
            cb.require_true("out_hi < 2^128".to_string(), in_range_hi);
            cb.require_equal(
                "overflow == add_gadget.overflow()".to_string(),
                overflow.expr(),
                add.overflow(),
            );
        });
