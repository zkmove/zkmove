use crate::execution_circuit::executions::{ExecutionState, Membership, DEPTH_POW_OF_ONE_LEVEL};
...
impl<F: Field, const VEC_UNPACK: bool> InstructionGadgetV2<F> for UnpackStage1<F, VEC_UNPACK> {
    ...
    fn configure(cb: &mut VmConstraintBuilder<F>) -> Self {
        ...
        let num_field = step_curr.operand1.expr();
    ...
    fn assign(
        ...
        let value = region.get_advice(offset, step.operand1.get_column_idx(), Rotation::cur());
        self.is_zero_num_field.assign(region, offset, value)?;
...
impl<F: Field, const VEC_UNPACK: bool> InstructionGadgetV2<F> for UnpackStage2<F, VEC_UNPACK> {
    ...
    fn assign(
        ...
        let field_index = header_pop.sub_index.to_vec().first().cloned().unwrap() as u64;
