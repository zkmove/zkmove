...
impl<F: Field, const N_LIMB: usize> Membership<F, N_LIMB> {
    ...
    pub(crate) fn configure(
        ...
        cb.require_equal(
            "header_sub_index == from_limbs(header_limbs)".to_string(),
            header_sub_index.clone(),
            from_limbs::expr::<_, _, N_BITS_ONE_LIMB>(&self.header_limbs),
        );
        cb.require_equal(
            "member_sub_index == from_limbs(&member_limbs)".to_string(),
        ...
        for i in 0..N_LIMB {
            cb.require_zero(
                "mask[i] * (header_limbs[i] * reverse_header_limbs[i] - 1) == 0".to_string(),
                self.mask[i].expr()
                    * (self.header_limbs[i].expr() * self.reverse_header_limbs[i].expr()
                        - 1u64.expr()),
            );
            cb.require_zero(
                "(1 - mask[i]) * header_limbs[i] == 0".to_string(),
                (1u64.expr() - self.mask[i].expr()) * self.header_limbs[i].expr(),
            );
            cb.require_zero(
                "mask[i] * (header_limbs[i] - member_limbs[i]) == 0".to_string(),
        ...
        cb.require_zero(
            "header_member_diff * reverse_header_member_diff - 1 == 0".to_string(),
...
impl<F: Field, const N_LIMB: usize> ExtendedSubIndex<F, N_LIMB> {
    pub(crate) fn construct(cb: &mut VmConstraintBuilder<F>, sub_index: Expression<F>) -> Self {
    ...
    pub(crate) fn configure(&self, cb: &mut VmConstraintBuilder<F>) {
        cb.require_equal(
            "sub_index == from_limbs(limbs)".to_string(),
        ...
        cb.require_zero(
            "sum(mask[i]) == 1 when sub_index != 0".to_string(),
            self.sub_index.clone() * (sum_mask - 1u64.expr()),
        );

        for i in 0..N_LIMB {
            cb.require_zero(
                "if mask[i] == 1, limbs[i] != 0".to_string(),
                self.mask[i].expr()
