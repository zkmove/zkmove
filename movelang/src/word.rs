...
use crate::value::{
    ...
    Location, PrimitiveValue, Reference, Value, ValueLocation, DEPTH_OF_ADDR_EXT,
    DEPTH_OF_LOCATION_PATH, U128,
};
    ...
    fn fold(word: Vec<(Vec<u128>, PrimitiveValue<F>)>) -> Self {
        ...
        for (i, (_, val)) in word.iter().skip(DEPTH_OF_LOCATION_PATH + 1).enumerate() {
            ...
            let x = val
                ...
                .get_lower_128();
            value += x << (16 * (DEPTH_OF_LOCATION_PATH + DEPTH_OF_ADDR_EXT - 1 - i));
        }
