...
pub fn load_package(rooted_path: &Path) -> Result<CompiledPackage> {
...
pub fn get_circuit_config_args_from_move_toml(
    toml_path: &Path,
...
/// Parse the entry call (module id + function name) declared in the `[circuit(.name)].entry`
/// section of `Move.toml`.
pub fn get_entry_call_from_move_toml(
    toml_path: &Path,
    circuit_name: Option<&str>,
    ...
    Ok((module_id, function_name.to_string()))
}

pub fn get_entry_info_from_move_toml(
    toml_path: &Path,
    circuit_name: Option<&str>,
) -> Result<EntryInfo> {
    let (module_id, function_name) = get_entry_call_from_move_toml(toml_path, circuit_name)?;
...
pub fn find_package_root(path: &Path) -> Result<PathBuf> {
    SourcePackageLayout::try_find_root(&path.canonicalize()?)
        .context("Failed to find root path for the package")
}

pub fn parse_module_id(module_id_str: &str) -> Result<ModuleId> {
...
pub fn save_to_file<P: AsRef<Path>, D: AsRef<[u8]>>(
    dir: P,
    file_name: &str,
    data: D,
) -> Result<()> {
