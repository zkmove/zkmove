...
impl CellType {
    ...
    pub fn expr_phase<F: Field>(meta: &mut ConstraintSystem<F>, expr: &Expression<F>) -> u8 {
        use Expression::*;
        match expr {
    ...
    pub fn storage_for_phase(phase: u8) -> CellType {
        match phase {
    ...
    pub fn storage_for_expr<F: Field>(
...
impl<F: Field> Cell<F> {
    ...
    #[cfg(feature = "test-circuits")]
    pub fn at_offset(&self, meta: &mut ConstraintSystem<F>, offset: i32) -> Self {
        Self::new_from_cs(meta, self.column, self.rotation + offset as isize)
    }
    #[cfg(not(feature = "test-circuits"))]
    pub fn at_offset(&self, offset: i32) -> Self {
        Self::new(self.column, self.rotation + offset as isize)
    }
}

impl<F> Cell<F> {
    pub fn get_column_idx(&self) -> usize {
        self.column.index()
    }
    pub fn get_column(&self) -> Column<Advice> {
        self.column
    }

    pub fn get_rotation(&self) -> isize {
        self.rotation
    }
}
...
pub trait CellPlacementStrategy {
...
#[derive(Clone, Debug)]
pub struct CellManager<S: CellPlacementStrategy> {
    strategy: S,
}
