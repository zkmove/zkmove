use crate::cell_manager::Cell;
use field_exts::util::Expr;
use field_exts::Field;
use halo2_proofs::plonk::Expression;

pub trait ConstraintBuilder<F: Field> {
    fn add_constraint(&mut self, name: String, constraint: Expression<F>);

    fn require_zero(&mut self, name: String, constraint: Expression<F>) {
        self.add_constraint(name, constraint);
    }

    fn require_equal(&mut self, name: String, lhs: Expression<F>, rhs: Expression<F>) {
        self.add_constraint(name, lhs - rhs);
    }

    fn require_boolean(&mut self, name: String, value: Expression<F>) {
        self.add_constraint(name, value.clone() * (1u64.expr() - value));
    }

    fn require_true(&mut self, name: String, value: Expression<F>) {
        self.require_equal(name, value, 1u64.expr());
    }

    fn require_in_set(&mut self, name: String, value: Expression<F>, set: Vec<Expression<F>>) {
        self.add_constraint(
            name,
            set.iter().fold(1u64.expr(), |acc, item| {
                acc * (value.clone() - item.clone())
            }),
        );
    }
    /// Under active development
    #[allow(dead_code)]
    fn add_constraints(&mut self, constraints: Vec<(String, Expression<F>)>) {
        for (name, constraint) in constraints {
            self.add_constraint(name, constraint);
        }
    }

    fn query_cell(&mut self) -> Cell<F> {
        unreachable!("query_cell should be implemented in the concrete ConstraintBuilder");
    }
    fn query_bool(&mut self) -> Cell<F> {
        unreachable!("query_bool should be implemented in the concrete ConstraintBuilder");
    }
    fn query_byte(&mut self) -> Cell<F> {
        unreachable!("query_byte should be implemented in the concrete ConstraintBuilder");
    }
    fn query_bytes<const N: usize>(&mut self) -> [Cell<F>; N] {
        unreachable!("query_bytes should be implemented in the concrete ConstraintBuilder");
    }
}

#[derive(Default)]
pub struct BaseConstraintBuilder<F> {
    pub constraints: Vec<(String, Expression<F>)>,
    pub max_degree: usize,
    pub condition: Option<Expression<F>>,
}

impl<F: Field> ConstraintBuilder<F> for BaseConstraintBuilder<F> {
    fn add_constraint(&mut self, name: String, constraint: Expression<F>) {
        let constraint = match &self.condition {
            Some(condition) => condition.clone() * constraint,
            None => constraint,
        };
        self.validate_degree(constraint.degree(), &name);
        self.constraints.push((name, constraint));
    }
}
impl<F: Field> BaseConstraintBuilder<F> {
    pub fn new(max_degree: usize) -> Self {
        BaseConstraintBuilder {
            constraints: Vec::new(),
            max_degree,
            condition: None,
        }
    }

    pub fn condition<R>(
        &mut self,
        condition: Expression<F>,
        constraint: impl FnOnce(&mut Self) -> R,
    ) -> R {
        debug_assert!(
            self.condition.is_none(),
            "Nested condition is not supported"
        );
        self.condition = Some(condition);
        let ret = constraint(self);
        self.condition = None;
        ret
    }

    pub fn validate_degree(&self, degree: usize, name: &str) {
        if self.max_degree > 0 {
            debug_assert!(
                degree <= self.max_degree,
                "Expression {} degree too high: {} > {}",
                name,
                degree,
                self.max_degree,
            );
        }
    }

    pub fn gate(&self, selector: Expression<F>) -> Vec<(String, Expression<F>)> {
        self.constraints
            .clone()
            .into_iter()
            .map(|(name, constraint)| (name, selector.clone() * constraint))
            .filter(|(name, constraint)| {
                self.validate_degree(constraint.degree(), name);
                true
            })
            .collect()
    }
}
